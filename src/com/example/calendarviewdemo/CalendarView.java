...
public class CalendarView extends View implements View.OnTouchListener {
	...
	private List<Date> undateList = new ArrayList<Date>();
	//给控件设置监听事件
...
//		canvas.drawRect(surface.weekHeight, surface.width, surface.weekHeight, surface.width, surface.cellBgPaint);
		...
		if (undateList != null && undateList.size() != 0) {
			...
			if (selectIndex != null && selectIndex.size() != 0) {
				for (int ielement : selectIndex) {
					drawCellBg(canvas, ielement, surface.bgColor);
				}
			}
		}
	...
	private void findundateIndex(int startIndex, int endIndex,
		...
		for (int i = startIndex; i < endIndex; i++) {
			...
			for (Date dateelement : undateList) {
				if (temp.compareTo(dateelement) == 0) {
					selectIndex.add(i);
				}
			}
		}
...
//					selectedStartDate = selectedEndDate = downDate;
					if (undateList != null && undateList.contains(downDate)) {
						undateList.remove(downDate);
					}else {
						undateList.add(downDate);
					}
	...
	public interface OnItemClickListener {
		void OnItemClick(Date selectedStartDate,Date selectedEndDate, List<Date> dates, boolean dis);
	}
